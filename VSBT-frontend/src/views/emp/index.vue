...
onMounted(async () => {
  search()
  //加载所有部门数据
  const result: any = await queryAllDeptApi()
  if (result.code) {
...
watch(
  ...
  (newValue, oldValue) => {
    ...
    console.log('监听-employee员工对象中的工作经历数据:', employee.value.exprList)
  },
  { deep: true },
)
...
// 表单保存按钮 - 保存新员工
const save = async () => {
  const valid: any = await employeeFormRef.value?.validate()
...
const deleteById = async (id: number) => {
  try {
    await ElMessageBox.confirm('您确认删除该员工吗?', '提示')

    const result: any = await deleteApi(id)
...
const handleSelectionChange = (selection: Emp[]) => {
  ...
  ElMessageBox.confirm('您确认删除该员工吗?', '提示', {
    ...
    .then(async () => {
      ...
      if (selectedIds.value && selectedIds.value.length > 0) {
        const result: any = await deleteApi(selectedIds.value)
        if (result.code) {
          ElMessage.success('删除成功')
          search()
        } else {
...
<template>
  ...
  <el-form :inline="true" :model="searchEmp">
    ...
    <el-form-item label="性别">
      <el-select v-model="searchEmp.gender" placeholder="请选择" style="width: 100px">
